...
class TransfuserFeatureBuilder(AbstractFeatureBuilder):
    ...
    def _get_lidar_feature(self, agent_input: AgentInput) -> torch.Tensor:
        ...
        :param agent_input: input dataclass
            - agent_input.lidars[-1].lidar_pc[LidarIndex.POSITION]: (3, N) numpy array
        :return: LiDAR histogram as torch tensors (2, H, W) or (1, H, W)
            셀 내에 포함된 LiDAR 포인트의 수를 나타냅니다. (0과 1 사이의 값)
            포인트 수가 너무 많을 경우 self._config.hist_max_per_pixel 값을 넘어가지 않도록 클리핑합니다.
            이후, 해당 셀의 값은 최대값으로 나누어 0과 1 사이로 정규화됩니다.

        max_height_lidar: float = 100.0
            - LiDAR 포인트의 최대 높이입니다. 이 높이보다 높은 포인트는 무시됩니다.
        lidar_split_height: float = 0.2
            - LiDAR 포인트를 두 그룹으로 나누는 데 사용되는 높이입니다.
            - use_ground_plane이 True인 경우, 이 높이보다 낮은 포인트는 아래쪽 그룹에 포함됩니다.
            - use_ground_plane이 False인 경우, 이 높이보다 낮은 포인트는 무시됩니다.
        lidar_min_x = -32 / lidar_max_x = 32
            - LiDAR 포인트의 x 좌표의 최소 및 최대 값입니다.
        lidar_min_y = -32 / lidar_max_y = 32
            - LiDAR 포인트의 y 좌표의 최소 및 최대 값입니다.
        """
