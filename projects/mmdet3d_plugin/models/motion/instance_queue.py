...
class InstanceQueue(nn.Module):
    def __init__(
        ...
        self.ego_feature_encoder = nn.Sequential(
            nn.Conv2d(embed_dims, embed_dims, 3, stride=1, padding=1, bias=False), # (1, 256, 8, 22)
            nn.BatchNorm2d(embed_dims),
            nn.Conv2d(embed_dims, embed_dims, 3, stride=2, padding=1, bias=False), # (1, 256, 4, 11)
            nn.BatchNorm2d(embed_dims),
            nn.ReLU(),
    ...
    def prepare_motion(
        ...
        if self.period == None:
            ...
            match = instance_id[..., None] == prev_instance_id[:, None] # [1, 900, 900]
            ...
            for i in range(len(self.instance_feature_queue)):
                ...
                ).sum(dim=2) # [B, 900, 256] -> 과거 900개 앵커 중, tracking이 되는 앵커(instance_feature_queue)만 남김
                self.instance_feature_queue[i] = temp_feature

                temp_anchor = self.anchor_queue[i]
                temp_anchor = (
                    match[..., None] * temp_anchor[:, None]
                ).sum(dim=2) # [B, 900, 11] -> 과거 900개 앵커 중, tracking이 되는 앵커(anchor_queue)만 남김
                self.anchor_queue[i] = temp_anchor

            self.period = (
                match * self.period[:, None]
            ).sum(dim=2) # [B, 900] -> 과거 900개 앵커 중, tracking이 되는 앵커의 period만 남김

        self.instance_feature_queue.append(instance_feature.detach()) # [1, 900, 256]
    ...
    `       feature_maps: List[Tensor]
        ...
        ego_feature = ego_feature.unsqueeze(1).squeeze(-1).squeeze(-1) # ( 1, 1, 256)

        ego_anchor = torch.tile(
            self.ego_anchor[None], (batch_size, 1, 1) #  (B, 1, 11)
        )
        ...
        if self.ego_period == None:
            self.ego_period = ego_feature.new_zeros((batch_size, 1)).long() # (B, 1)
        else:
