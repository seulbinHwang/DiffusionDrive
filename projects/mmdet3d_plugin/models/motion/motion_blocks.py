...
class MotionPlanningRefinementModule(BaseModule):
    ...
    def forward(
        ...
        planning_status = self.plan_status_branch(ego_feature +
                                                  ego_anchor_embed)
    ...
    def infer_postprocess(self, plan_reg, bs, metas):
        ...
        reg_pred = plan_reg.reshape(bs, 3, 1, self.ego_fut_mode,
                                    self.ego_fut_ts, 2)
        reg_pred = reg_pred[bs_indices, cmd]
        best_reg = get_best_reg(reg_pred, gt_reg_target, gt_reg_mask)
        plan_reg = best_reg.view(bs, 1, 1, self.ego_fut_ts,
                                 2).repeat(1, 1, 3 * self.ego_fut_mode, 1, 1)
