...
class HierarchicalPlanningDecoder(object):
    ...
    def decode(
        ...
        classification = planning_output['classification'][-1] # bs, 1, cmd_mode*modal_mode
        prediction = planning_output['prediction'][-1] # bs,1,cmd_mode*modal_mode,ego_fut_ts,2
...
class V1HierarchicalPlanningDecoder(object):
    ...
    def decode(
        ...
        prediction = prediction[:,None,None,:,:].repeat(1,3,self.ego_fut_mode,1,1)
...
class V2HierarchicalPlanningDecoder(HierarchicalPlanningDecoder):
    ...
    def decode(
        ...
        prediction = planning_output['prediction'][-1] # bs,modal_mode,ego_fut_ts,2
        bs = classification.shape[0]
        classification = classification.reshape(bs,1,self.ego_fut_mode).repeat(1,3,1)
        prediction = prediction.reshape(bs,1,self.ego_fut_mode, self.ego_fut_ts, 2).cumsum(dim=-2).repeat(1,3,1,1,1)
        # classification: bs, 3, modal_mode
        # final_planning: bs, ego_fut_ts, 2
        classification, final_planning = self.select(det_output, motion_output, classification, prediction, data)
        anchor_queue = planning_output["anchor_queue"]
