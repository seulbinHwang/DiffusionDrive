...
class BEVRender:
    ...
    def draw_planning_anchor(self, data):
        ...
        plan_vocabulary = np.concatenate((np.zeros(
            (vocabulary.shape[0], 1, vocabulary.shape[-1])), vocabulary),
                                         axis=1)
    ...
    def draw_detection_pred(self, result):
        if not (self.plot_choices['draw_pred'] and self.plot_choices['det'] and
                "boxes_3d" in result):
            return
        ...
        for i in range(result['labels_3d'].shape[0]):
            ...
            if score < SCORE_THRESH:
                continue
            color = color_mapping[result['instance_ids'][i] %
                                  len(color_mapping)]
    ...
    def draw_track_pred(self, result):
        if not (self.plot_choices['draw_pred'] and
                self.plot_choices['track'] and "anchor_queue" in result):
            return
        ...
        for i in range(result['labels_3d'].shape[0]):
            ...
            for j in range(period[i]):
                # draw corners
                corners = box3d_to_corners(temp_bboxes[:,
                                                       -1 - j])[i,
                                                                [0, 3, 7, 4, 0]]
                x = corners[:, 0]
                y = corners[:, 1]
    ...
    def draw_motion_pred(self, result, top_k=3):
        if not (self.plot_choices['draw_pred'] and
                self.plot_choices['motion'] and "trajs_3d" in result):
            return
    ...
    def draw_map_pred(self, result):
        if not (self.plot_choices['draw_pred'] and self.plot_choices['map'] and
                "vectors" in result):
            return
    ...
    def draw_planning_gt(self, data):
        ...
        if masks[0] != 0:
            ...
            plan_traj = np.concatenate((np.zeros(
                (1, plan_traj.shape[1])), plan_traj),
                                       axis=0)
            self._render_traj(plan_traj,
                              traj_score=1.0,
                              colormap='autumn',
                              dot_size=50)
    ...
    def draw_diffusing_gt(self, data):
        ...
        plan_traj = np.concatenate((np.zeros(
            (plan_traj.shape[0], 1, plan_traj.shape[2])), plan_traj),
                                   axis=1)
        for i, traj in enumerate(plan_traj):
            self._render_traj(traj,
                              traj_score=1.0,
                              colormap='autumn',
                              dot_size=500,
                              points_per_step=3)
    ...
    def draw_planning_vocabulary_v2(self, data):
        ...
        # Add zero as starting point for each trajectory
        plan_vocabulary = np.concatenate((np.zeros(
            (self.vocabulary.shape[0], 1, self.vocabulary.shape[-1])),
                                          self.vocabulary),
                                         axis=1)
        ...
        for i, traj_idx in enumerate(sampled_indices):
            ...
            opacity = min(0.8,
                          max(0.2,
                              traj_length / 40.0))  # Scale opacity with length
    ...
    def draw_planning_anchor_v2(self, data):
        ...
        for i, anchor_traj in enumerate(plan_vocabulary):
            ...
            self._render_traj(anchor_traj,
                              traj_score=1.0,
                              colormap=colormap,
                              dot_size=50,
                              points_per_step=20)
            line_color = matplotlib.colormaps[colormap](
                0.8)[:3]  # Using 0.8 to get a slightly darker shade
            self.axes.plot(
    ...
    def draw_denoised_anchor(self, data):
        ...
        if masks[0] != 0:
            ...
            gt_plan_traj = np.concatenate((np.zeros(
                (1, gt_plan_traj.shape[1])), gt_plan_traj),
                                          axis=0)
        ...
        for i, anchor_traj in enumerate(plan_vocabulary):
            ...
            if masks[0] != 0:
                ...
                if i == np.argmin([
                        np.linalg.norm(traj - gt_plan_traj, axis=1).mean()
                        for traj in plan_vocabulary
                ]):
                    anchor_opacity = 1.0
                    zorder = 3
                    colormap = 'autumn'
    ...
    def draw_planning_pred(self, data, result, top_k=3):
        if not (self.plot_choices['draw_pred'] and
                self.plot_choices['planning'] and "planning" in result):
            return
        ...
        if self.plot_choices['track'] and "ego_anchor_queue" in result:
            ...
            for j in range(ego_period[0]):
                # draw corners
                corners = box3d_to_corners(
                    ego_temp_bboxes[:, -1 - j])[0, [0, 3, 7, 4, 0]]
                x = corners[:, 0]
                y = corners[:, 1]
                self.axes.plot(x,
                               y,
        ...
        plan_trajs = np.concatenate((np.zeros(
            (num_cmd, num_mode, 1, 2)), plan_trajs),
                                    axis=2)
